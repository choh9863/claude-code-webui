...
## 4. `node-pty-prebuilt-multiarch` still lacks Node 22 binaries
- **File:** `backend/package.json`
- **Issue:** The follow-up change that aliases `node-pty` to `node-pty-prebuilt-multiarch` avoids compiling from source, but that fork only publishes prebuilt artifacts up to Node 20. With Node 22.17.1 the install script prints `Prebuild binary missing for platform` and exits after trying to spawn `prebuild-install`, so Windows contributors still hit a hard failure during `npm install`.
- **Recommendation:** Either downgrade the required Node version to a release with available prebuilds (≤20.x), switch back to `node-pty` and document the native build prerequisites, or replace the dependency with one that officially supports Node 22 (for example by vendoring a newer fork or removing the CLI integration on Windows).

## 5. Background sessions drop streamed updates in the UI
